...
namespace OdevAPI.Common.Logging;
...
public sealed class CustomJsonFormatter : ITextFormatter
{
    public void Format(LogEvent logEvent, TextWriter output)
    {
        ...
        using (var writer = new Utf8JsonWriter(stream, new JsonWriterOptions { Indented = false }))
        {
            ...
            writer.WriteString("service", GetString(logEvent, "Application") ?? "library-mgmt");
            ...
            var loanId = GetLong(logEvent, "LoanId");
            if (loanId.HasValue)
                writer.WriteNumber("loanId", loanId.Value);

            var bookId = GetLong(logEvent, "BookId");
            if (bookId.HasValue)
                writer.WriteNumber("bookId", bookId.Value);
